{def $zone_names = ezini( $attribute.content.zone_layout, 'ZoneName', 'zone.ini' )
	 $allowed_zones = fetch('ezmodule', 'allowed_zones')
	 $can_change_layout = fetch( 'user', 'has_access_to', hash( 'module', 'ezmodule', 'function', 'changelayout' ) ) }

<div id="page-datatype-container" class="yui-skin-sam yui-skin-ezmodule">
	{if $can_change_layout}
...
{ezscript( array( 'ezjsc::yui2', 'ezjsc::yui3', 'ezjsc::yui3io' ) )}
...
(function() {ldelim}
	YUILoader.onSuccess = function() {ldelim}
		...
		{foreach $attribute.content.zones as $index => $zone}
			...
			tabView.addTab( new YAHOO.widget.Tab({ldelim}
				...
				dataSrc: '{concat( '/ezmodule/zone/', $attribute.id, '/', $attribute.version, '/', $index  )|ezurl(no)}',
				cacheData: true
				{rdelim}));
		{/foreach}
	...
	YUILoader.require(["button","calendar","container","cookie","get","json","tabview","utilities","blocktools","zonetools","scheduledialog", "pagedatatype-css"]);
...
{undef $zone_names $allowed_zones $can_change_layout}