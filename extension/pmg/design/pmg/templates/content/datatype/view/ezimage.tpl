...
{if $image_content.is_valid}
	...
	{if $link_to_image}
		{set $href = $image_content['original'].url|ezurl}
	{else}	
		{if $href}
			{if is_numeric($href)}
				{def
					$n = fetch('content', 'node', hash('node_id', $href))
				}
				
				{if and($n, $n.object.class_identifier|eq('file'))}
					{set
						$attribute = $n.data_map.file
						$href = concat('/content/download/', $attribute.contentobject_id, '/', $attribute.id,'/version/', $attribute.version , '/file/', $attribute.content.original_filename|urlencode)
					}
				{/if}
				{undef $n}
			{/if}
	...
	{if is_set($target)}
		{if eq($target, '_blank')}
			{set $link_class = concat($link_class, 'external')}
		{/if}
	{/if}
	
	{switch match=$alignment}
		{case match='left'}<div class="imageleft">{/case}
		{case match='right'}<div class="imageright">{/case}
		{case/}
	{/switch}

	{if $css_class}<div class="{$css_class|wash}">{/if}
	...
	{if and( is_set( $image ), $image )}
		...
		{if $href}<a href="{$href}"{if $link_class|trim|ne('')} class="{$link_class}"{/if}{if and( is_set( $link_id ), $link_id )} id="{$link_id}"{/if}>{/if}
		<img src={$image.url|ezroot} width="{$image.width}" height="{$image.height}"{if $hspace} hspace="{$hspace}"{/if}{if $inline_style|trim|ne('')} style="{$inline_style}"{/if} alt="{$alt_text|wash(xhtml)}" title="{$title|wash(xhtml)}" />
		{if $href}</a>{/if}
	{/if}
	...
	{switch match=$alignment}
		{case match='left'}</div>{/case}
		{case match='right'}</div>{/case}
		{case/}
	{/switch}
